...
- how to define DP state (what var do we need? here we need 2 var: index & color)
    - prefer suffix (as MIT 6.006)
- return the optimal solution using memo-dfs
"""
    ...
    def minCost(self, costs: List[List[int]]) -> int:
        def os_tabular():
            ...
            for n in range(N - 2, -1, -1):
                F[n % 2][0] = min([F[(n + 1) % 2][c] for c in [1, 2]]) + costs[n][0]
                F[n % 2][1] = min([F[(n + 1) % 2][c] for c in [0, 2]]) + costs[n][1]
                F[n % 2][2] = min([F[(n + 1) % 2][c] for c in [0, 1]]) + costs[n][2]
                print(F)
            if len(costs) == 0:
                return 0
            return min(F[0])
